...
export default class PubSub {
  ...
  unsubscribe(event: PropertyKey, callback: any): boolean {
    let self = this;
    ...
    return self.events[event].splice(self.events[event].indexOf(callback), 1);
  }
  /**
   ...
   * @memberof PubSub
   */
  subscribe(event: PropertyKey, callback: any): number {
    let self = this;
   ...
   * @memberof PubSub
   */
  publish(event: PropertyKey, data: object = {}): Array<any> {
    let self = this;
    ...
    return self.events[event].map((callback: (arg0: object) => any) => callback(data));
  }
}
